...
class notch_port(circlefit, save_load, plotting, calibration):
	...
	def GUIfit_Manual(self):
		...
		f=self.f_data
		zraw=self.z_data_raw
		zsim=self.z_data_sim

		l0, = ax0.plot(f*1e-9,np.absolute(zraw),'o')
		l0s, = ax0.plot(f*1e-9,np.absolute(zsim))
		...
		axa= plt.axes([0.25, 0.075, 0.65, 0.03], facecolor=axcolor)
		axalpha = plt.axes([0.25, 0.1, 0.65, 0.03], facecolor=axcolor)
		axfr= plt.axes([0.25, 0.125, 0.65, 0.03], facecolor=axcolor)
		axQi= plt.axes([0.25, 0.15, 0.65, 0.03], facecolor=axcolor)
		axQc= plt.axes([0.25, 0.175, 0.65, 0.03], facecolor=axcolor)
		axphi0= plt.axes([0.25, 0.2, 0.65, 0.03], facecolor=axcolor)


		sdelay = Slider(axdelay, 'delay', 0.5*self.fitresults["delay"], 2*self.fitresults["delay"], valinit=self.fitresults["delay"])
		sa = Slider(axa, 'a',0.5*self.fitresults["a"], 2*self.fitresults["a"], valinit=self.fitresults["a"])
		salpha = Slider(axalpha, 'alpha',2*self.fitresults["alpha"], 0.5*self.fitresults["alpha"], valinit=self.fitresults["alpha"])

		sfr = Slider(axfr, 'fr', self.fitresults['fr']*1e-9-0.002, self.fitresults['fr']*1e-9+0.002, valinit=self.fitresults['fr']*1e-9)
		sQi = Slider(axQi, 'Qi',self.fitresults['Qi_dia_corr']/2, self.fitresults['Qi_dia_corr']*2, valinit=self.fitresults['Qi_dia_corr'])
		sQc = Slider(axQc, 'Qc', self.fitresults['absQc']/2, self.fitresults['absQc']*2, valinit=self.fitresults['absQc'])
		sphi0 = Slider(axphi0, 'phi0', -np.pi, np.pi, valinit=self.fitresults['phi0'])


		def update(val):
			Ql=1/(1/sQi.val+1/sQc.val)
			l0s.set_ydata(np.absolute(sa.val*np.exp(np.complex(0,salpha.val))*np.exp(-2j*np.pi*f*sdelay.val)*(1.-Ql/sQc.val*np.exp(1j*sphi0.val)/(1.+2j*Ql*(f-sfr.val*1e9)/(sfr.val*1e9)))))
		def save(val):
			Ql=1/(1/sQi.val+1/sQc.val)
			self.z_data_sim = self._S21_notch(self.f_data,fr=sfr.val*1e9,Ql=Ql,Qc=sQc.val,phi=sphi0.val,a=sa.val,alpha=salpha.val,delay=sdelay.val)
			l0s.set_data(self.f_data*1e-9,np.absolute(self.z_data_sim))
		

		sdelay.on_changed(update)
		sa.on_changed(update)
		salpha.on_changed(update)
		sfr.on_changed(update)
		sQi.on_changed(update)
		sQc.on_changed(update)
		sphi0.on_changed(update)
		#make a button to reset the parameters if necessary
		...
		saveax = plt.axes([0.1, 0.065, 0.1, 0.04])
		button2 = Button(saveax, 'Save', color=axcolor, hovercolor='0.975')
		button2.on_clicked(save)
		update2ax = plt.axes([0.1, 0.105, 0.1, 0.04])
		button3 = Button(update2ax, 'Update plot', color=axcolor, hovercolor='0.975')
