...
class Switchs(ButtonsAndEntries):
    def __init__(self,canvas,button_width=50,button_height=50,label_width=50,label_height=80,x=160,y=140):

        self.button=tk.Button(self.canvas.root, text="On", command=lambda: self.switch())
        self.canvas.create_window(self.canvas.posx+x, self.canvas.posy+140,width=button_width,height=button_height,
                                   window=self.entry_list[0])
        self.canvas.create_window(self.canvas.posx+x, self.canvas.posy+120,width=label_width,height=label_height,
                                   window=tk.Label(self.canvas.root, text="Switchs",font=NORM_FONT))    
