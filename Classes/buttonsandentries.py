...
class ButtonsAndEntries:
    ...
    def radiobutton(self,parameter):
...
class Switchs(ButtonsAndEntries):
    def __init__(self,canvas,parameter,text,button_width=50,button_height=50,label_width=50,label_height=80,y_button=20):
        ...
        canvas.create_window(canvas.posx, canvas.posy,width=label_width,height=label_height,
                                   window=tk.Label(self.canvas.root, text=text,font=NORM_FONT))
class RadioButtons(ButtonsAndEntries):
    def __init__(self,canvas,parameter,possible_values,text,button_width=50,button_height=50,label_width=50,label_height=80,y_button=20):
        self.v = tk.StringVar(value=canvas.controller.parameters[parameter])
        self.button_list=[]
        for i in range(len(possible_values)):
            self.button_list.append(tk.Radiobutton(canvas.root,indicatoron=0,
                                                   text=possible_values[i], variable=self.v, value=possible_values[i],command=lambda: self.radiobutton(parameter)))
...
class Entries(ButtonsAndEntries):
    def __init__(self,canvas,parameter_list,labels_list,type_list,text,button_width=50,button_height=50,label_width=50,label_height=80,y_button=20):
    ...
    def submit(self,plot=True,load_baseline=False):
        if self.type=="int":
            self.canvas.controller.parameters[self.parameter]=int(self.entry.get())
        if self.type=="float":
            self.canvas.controller.parameters[self.parameter]=float(self.entry.get())
        if self.type=="str":
            self.canvas.controller.parameters[self.parameter]=str(self.entry.get())
        if self.type=="bool":
            self.canvas.controller.parameters[self.parameter]=bool(self.entry.get())
        if self.type=="np.array":
            self.canvas.controller.parameters[self.parameter]=np.linspace(float(self.entry_list[0].get()),float(self.entry_list[1].get()),int(self.entry_list[2].get()))
        if self.type=="radiobutton":
            self.canvas.controller.parameters[self.parameter]=self.v.get()
        if load_baseline==True:
            self.canvas.root.master.load_data(reset=False)
            self.canvas.root.master.load_baseline()
            self.canvas.root.master.smooth_data()
